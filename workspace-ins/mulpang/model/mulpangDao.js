...
// 쿠폰 상세 조회
module.exports.couponDetail = async (socketio, _id) => {
  ...
  socketio.emit('top5', top5);

  console.log(coupon.couponName, coupon.viewCount);
  return coupon;
};
...
module.exports.login = async (params) => {
	// 지정한 아이디와 비밀번호로 회원 정보를 조회한다.
  let result;
	try{
    result = await db.member.findOne(params, { projection: { profileImage: 1 } });
  }catch(err){
    console.error(err);
    throw new Error('작업 처리에 실패했습니다. 잠시후 다시 요청하시기 바랍니다.');
  }
