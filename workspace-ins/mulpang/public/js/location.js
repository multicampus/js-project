...
function addCouponToMap(){
  ...
  articleList.each(function() {
    ...
    const position = { lat: article.data('lat'), lng: article.data('lng') };
    ...
    marker.addListener('click', function(){
      ...
      openWindow?.close(); // optional chaning
      info.open(map);
      openWindow = info;
    });
	});
	...
	google.maps.event.addListenerOnce(map, 'tilesloaded', function() {
		showSlider();
		// 2.3 지도의 범위가 변경될 경우 지도안의 쿠폰을 슬라이더에 보여주도록 이벤트 추가
		map.addListener('zoom_changed', showSlider); // 축척 변경
		map.addListener('dragend', showSlider); // 드래그 완료
